...
function(target_commonlibsse_properties TARGET)
    ...
    # Setup struct compatibility call.
    if (NOT ADD_COMMONLIBSSE_PLUGIN_STRUCT_DEPENDENT)
        set(commonlibsse_plugin_struct_call "v.UsesNoStructs()")
    else ()
        set(commonlibsse_plugin_struct_call "v.UsesUpdatedStructs()")
    endif ()

    # Setup version independence call.
    ...
    if (ADD_COMMONLIBSSE_PLUGIN_USE_ADDRESS_LIBRARY OR ADD_COMMONLIBSSE_PLUGIN_USE_SIGNATURE_SCANNING)
        ...
        if (NOT ADD_COMMONLIBSSE_PLUGIN_USE_ADDRESS_LIBRARY)
            set(commonlibsse_plugin_compat_call "v.UsesSigScanning()")
        else ()
            set(commonlibsse_plugin_compat_call "v.UsesAddressLibrary()")
        endif ()
    else ()
        ...
        set(commonlibsse_compat_versions "")
        foreach (SKYRIM_VERSION ${ADD_COMMONLIBSSE_PLUGIN_COMPATIBLE_RUNTIMES})
            if (NOT "${commonlibsse_compat_versions}" STREQUAL "")
                set(commonlibsse_compat_versions "${commonlibsse_compat_versions}, ")
            endif ()
            ...
            set(commonlibsse_compat_versions "${commonlibsse_compat_versions}REL::Version{ ${COMMONLIBSSE_VERSION_MAJOR}, ${COMMONLIBSSE_VERSION_MINOR}, ${COMMONLIBSSE_VERSION_PATCH}, ${COMMONLIBSSE_VERSION_TWEAK} }")
        endforeach ()
        set(commonlibsse_plugin_compat_call "v.CompatibleVersions({ ${commonlibsse_compat_versions} })")
    endif ()

    file(WRITE "${commonlibsse_plugin_file}"
            "#include \"SKSE/SKSE.h\"\n"
            "\n"
            "static consteval SKSE::PluginVersionData InitPluginVersionData() noexcept {\n"
            "    SKSE::PluginVersionData v{};\n"
            "    v.PluginVersion(${commonlibsse_plugin_version});\n"
            "    v.PluginName(\"${commonlibsse_plugin_name}\");\n"
            "    v.AuthorName(\"${ADD_COMMONLIBSSE_PLUGIN_AUTHOR}\");\n"
            "    v.AuthorEmail(\"${ADD_COMMONLIBSSE_PLUGIN_EMAIL}\");\n"
            "    ${commonlibsse_plugin_struct_call};\n"
            "    ${commonlibsse_plugin_compat_call};\n"
            "    v.MinimumRequiredXSEVersion(${commonlibsse_min_skse_version});\n"
            "    return v;\n"
            "}\n"
            "\n"
            "SKSEPluginVersion { InitPluginVersionData() };\n")
